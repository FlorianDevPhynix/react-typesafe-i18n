...
import { DomHtmlParser } from './dom_parser';
import {
	Attributes,
...
describe('DomHtmlParser', () => {
	...
	test('basic', () => {
		const result = DomHtmlParser.parseHtml(
	...
	test('with propertys', () => {
		const result = DomHtmlParser.parseHtml(
			'Some text <p id="first" class="test" id="second">with a html element</p> and more text.',
			filterAttributes,
			filterElementTags
		);
	...
	test('no propertys on close tag', () => {
		const result = DomHtmlParser.parseHtml(
